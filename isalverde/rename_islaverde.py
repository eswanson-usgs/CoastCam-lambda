"""
Eric Swanson
Purpose: Convert the filename for an image at Isla Verde to the proper argus-style format.
"""
...
def rename_islaverde(args):
    ...
    if isImage == True:
        ...
        if len(filename_elements) == 10:
            ...
            if filename_station != 'islaverde':
                new_filepath = source_filepath.replace(filename_station, 'islaverde')
...
with concurrent.futures.ThreadPoolExecutor() as executor:
    results = executor.map(rename_islaverde, args)
