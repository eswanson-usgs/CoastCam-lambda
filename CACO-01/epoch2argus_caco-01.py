"""
Eric Swanson
Purpose: Convert the filename for an image at Caco-01 from the epoch only format to the argus format filename.
...
The old filepath is in the format s3:/cmgp-coastcam/cameras/caco-01/products/[long filename].
The new filepath is in the format s3:/cmgp-coastcam/cameras/caco-01]/[camera]/[year]/[day]/raw/[longfilename].
...
### for images in new_products/products folder in S3 ###
source_folder = "s3://cmgp-coastcam/cameras/caco-01/new_products/products"                        
image_list = file_system.glob(source_folder+'/*')
args = ((image, station) for image in image_list)
with concurrent.futures.ThreadPoolExecutor() as executor:
    results = executor.map(epoch2argus, args)
