...
Lambda function is triggered by image uploaded to the Sunset Beach S3 coastcam bucket at with the prefix ('directory') cameras/sunset/products/. Lambda function will not trigger 
...
def get_new_keys(old_key):
    ...
    cameras/[station]/dumps/dumps/[long filename]. The new key will have the format
    cameras/[station]/[camera]/[year]/[day]/raw/[filename]
    There will also be a second key generated that renames the file in the old filepath (/dumps/dumps directory) with a new name that matches the format rest of the CoastCam cameras.
    ...
    Output:
        ...
        new_dumps_key - (string) filepath with a renamed file for the /dumps/dumps/ directory
    '''
    ...
    if len(filename_elements) != 4:
        ...
        new_key = "cameras/sunset/" + cam_num + "/" + year + "/" + new_format_day + '/' + new_filename
        new_product_key = "cameras/sunset/products/" + new_filename
...
def lambda_handler(event='none', context='none'):
    ...
    print('key for new filename in /products:', new_dumps_key)
