#Email alert LAmbda function for Sunset Beach, CA. Triggers once a day at 10 AM local and checks if any new imagery made it to the /latest folder. Does so by checking the csv file
...
def lambda_handler(event, context):
    ...
    csv_key = "cameras/sunset/sunset_most_recent_time.csv"
    
    #need to download csv to temp folder
    download_path = '/tmp/sunset_most_recent_time.csv'
    ...
    latest_prefix = "cameras/sunset/latest"
    ...
    for file in bucket.objects.filter(Prefix=latest_prefix):
        ...
        if file.last_modified.replace(tzinfo=None) > newest_latest_obj.replace(tzinfo=None):
            ...
            recent_time = str(newest_latest_obj.timestamp())
